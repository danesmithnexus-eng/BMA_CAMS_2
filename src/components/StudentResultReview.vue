...
export default {
    ...
    methods: {
        ...
        // FIX: extract raw answer value from an entry object using all possible field names
        extractRawValue(entry) {
        ...
        getAnswerEntry(question, sr) {
            ...
            if (Array.isArray(answers)) {
                ...
                if (found) return {
                    // FIX: check all possible field names when building the normalized entry
                    answer_value: found.answer_value ?? found.answer ?? found.selected ?? found.selected_option ?? found.choice ?? '',
        ...
        getStudentAnswerValue(question, sr) {
            ...
            // FIX: use extractRawValue instead of only checking answer_value
            const val = this.extractRawValue(entry);
        ...
        isMcStudentAnswer(question, idx, sr) {
            ...
            if (/^[A-Z]$/i.test(val)) {
                return (val.toUpperCase().charCodeAt(0) - 65) === idx;
            }
            ...
            // Match by numeric index — support both 0-based and 1-based
            const asNum = parseInt(val, 10);
            if (!isNaN(asNum) && String(asNum) === val) {
                if (asNum === idx) return true;      // 0-based
                if (asNum - 1 === idx) return true;  // 1-based
            }
